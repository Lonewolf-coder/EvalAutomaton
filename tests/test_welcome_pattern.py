...
def _make_task(**kwargs) -> TaskDefinition:
    defaults = dict(
        ...
        required_menu_items=["Book Flight", "Fetch Booking", "FAQ"],
    )
...
class TestWelcomePattern:
    ...
    @pytest.mark.asyncio
    async def test_passes_when_greeting_and_menu_present(self):
        task = _make_task()
    ...
    @pytest.mark.asyncio
    async def test_fails_when_menu_item_missing(self):
        task = _make_task()
        # Missing "Fetch Booking"
    ...
    async def test_per_item_checks_emitted(self):
        ...
        assert len(menu_checks) == 3
        assert all(c.status == CheckStatus.PASS for c in menu_checks)

    @pytest.mark.asyncio
    async def test_no_menu_requirement_passes_menu(self):
        task = _make_task(required_menu_items=[])
    ...
    async def test_second_turn_used_when_menu_not_in_first_response(self):
        ...
        task = _make_task(required_menu_items=["Book Flight", "FAQ"])
