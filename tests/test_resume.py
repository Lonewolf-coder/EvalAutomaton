...
class TestResumeMechanism:
    ...
    @pytest.mark.asyncio
    async def test_resume_raises_if_no_scorecard(self, tmp_path):
        engine = _make_engine(tmp_path)
        with pytest.raises(FileNotFoundError, match="Scorecard not found"):
            await engine.resume_evaluation("nonexistent-id")

    @pytest.mark.asyncio
    async def test_resume_raises_if_no_context(self, tmp_path):
        engine = _make_engine(tmp_path)
        ...
        with pytest.raises(FileNotFoundError, match="RuntimeContext not found"):
            await engine.resume_evaluation(session_id)
